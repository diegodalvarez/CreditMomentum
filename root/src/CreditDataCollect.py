...
class CreditDataCollect:
    ...
    def __init__(self):
        ...
        # *revert 
        self.bbg_data_path = r"/Users/diegoalvarez/Desktop/BBGData/root/BBGtickers.xlsx"
    ...
    def _get_cdx_rtn(self, df: pd.DataFrame) -> pd.DataFrame: 
        
        df_out = (df.sort_values(
            "date").
            assign(
                px_diff = lambda x: x.value.diff(),
                px_rtn  = lambda x: x.value.pct_change()))
        
        return df_out
    
    
    def get_cds_rtn(self, verbose: bool = False) -> pd.DataFrame: 
        
        file_path = os.path.join(self.raw_path, "CDXReturns.parquet")
        try:
            
            if verbose == True: print("Looking for CDX returns")
            df_out = pd.read_parquet(path = file_path, engine = "pyarrow")
            if verbose == True: print("Found Data\n")
...
def main() -> None:     
    ...
    CreditDataCollect().get_cds_rtn(verbose = True)
